...
| 版本 | 日期 | 说明（≤10字） | 提交哈希 |
|------|------|---------------|----------|
| v1.1.4 | 2026-01-18 | 修复未完全启动时继续 | 2728222 |
| v1.1.3 | 2026-01-18 | 修复继续生成卡queue | d4a5758 |
...
### Bug #5: 后端未完全启动时继续生成卡queue (已修复 - 2026-01-18)

**问题描述**:
- 后端重启但未完全准备好时，用户点击"继续生成剩余的x张"
- 对话框消失，但 WebSocket 连接失败
- 恢复状态已被清除，占位符卡在 `queue`，且无法再次恢复

**根本原因**:
- `handleContinueGeneration()` 在调用 `generateForPrompt()` **之前**就清除了恢复状态
- 如果后端未完全启动，WebSocket 连接失败
- 但恢复状态已经清除，用户无法再次点击"继续生成"
- 占位符卡在 `queue` 状态

**修复方案**:
1. 添加 `recoveryStateRef` 用于同步访问最新的恢复状态
2. 修改 `handleContinueGeneration()`：不立即清除恢复状态
3. 修改 `generateLoop()` 和 `generateBatch()`：WebSocket 连接成功后才清除恢复状态
4. 如果连接失败，恢复状态保持不变，`pauseGeneration()` 会重新暂停占位符

**修改文件**:
- `frontend/src/App.jsx` (行 181, 340-343, 1146-1147, 1584-1600, 1311-1327)

**修复后行为**:
- 用户点击"继续生成" → 占位符变为 `queue`，但恢复状态保持
- WebSocket 连接成功 → 清除恢复状态，对话框消失，正常继续生成
- WebSocket 连接失败 → 恢复状态保持，占位符重新变回 `paused`，对话框保持显示
- 用户可以等待后端完全启动后再次点击"继续生成"

**测试要求**:
- 后端下线，点击"继续生成" → 占位符变为 `queue` 但立即变回 `paused`
- 对话框不消失，可以再次点击
- 后端上线后，点击"继续生成" → 对话框消失，正常继续生成
